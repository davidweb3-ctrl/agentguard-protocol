...
const htmlPath = path.join(repoRoot, "docs/demo-clip/index.html");
const framesDir = path.join(repoRoot, "docs/demo-clip/frames");
const outputPath = path.join(repoRoot, "docs/demo-clip/agentguard-mvp-demo.mp4");
...
for (let index = 0; index < frameCount; index += 1) {
  ...
  if ((index + 1) % 60 === 0 || index === frameCount - 1) {
...
if (process.env.KEEP_DEMO_FRAMES !== "1") {
  await rm(framesDir, { recursive: true, force: true });
}
